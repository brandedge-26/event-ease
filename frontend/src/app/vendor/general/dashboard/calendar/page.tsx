...
type DbBooking = {
  id: string; customerName: string; event: string;
  timeFrom: string | null; status: "confirmed" | "pending" | "cancelled" | "blocked";
  date: string;
};
...
// ─── Day Detail Modal ─────────────────────────────────────────────────────────
function DayDetailModal({ dateKey, bookings, onClose, onNewBooking }: {
  dateKey: string;
  bookings: CalBooking[];
  onClose: () => void;
  onNewBooking: () => void;
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
          {bookings.length === 0 && (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <button onClick={onNewBooking} className="mt-4 px-5 py-2.5 rounded-xl text-sm font-semibold cursor-pointer transition-opacity hover:opacity-80" style={{ background: "var(--primary)", color: "#ffffff" }}>
                + New Booking
              </button>
            </div>
          )}
          ...
          {bookings.length > 0 && (
            <button onClick={onNewBooking} className="w-full py-2.5 rounded-xl text-sm font-semibold cursor-pointer transition-opacity hover:opacity-80 mt-1" style={{ background: "var(--primary)", color: "#ffffff" }}>
              + New Booking
            </button>
          )}
        </div>
...
export default function GeneralCalendarPage() {
  ...
  function buildMapFromRaw(raw: DbBooking[]): BookingMap {
    ...
    raw.filter(b => b.status !== "cancelled").forEach(b => {
      const entry: CalBooking = {
        id:           b.id,
        customerName: b.customerName,
        event:        b.event,
        time:         fmt12h(b.timeFrom),
        status:       b.status as "confirmed" | "pending" | "blocked",
      };
    ...
    for (const p of pending) {
      ...
      if (p.method === "DELETE") {
        result = Object.fromEntries(
          Object.entries(result).map(([date, entries]) => [date, entries.filter(e => e.id !== bid)])
        );
        // Remove empty date keys
        result = Object.fromEntries(Object.entries(result).filter(([, entries]) => entries.length > 0));
      } else if (p.method === "PATCH") {
        const pl = p.payload;
        result = Object.fromEntries(
          Object.entries(result).map(([date, entries]) => [
            date,
            entries.map(e => e.id !== bid ? e : {
              ...e,
              ...(pl.customerName !== undefined ? { customerName: String(pl.customerName) } : {}),
              ...(pl.event        !== undefined ? { event:        String(pl.event) }        : {}),
              ...(pl.status       !== undefined ? { status:       pl.status as CalBooking["status"] } : {}),
            }),
          ])
        );
        // If date changed, move entry
        if (pl.date && typeof pl.date === "string") {
          const oldEntry = Object.values(result).flat().find(e => e.id === bid);
          if (oldEntry) {
            result = Object.fromEntries(
              Object.entries(result).map(([date, entries]) => [date, entries.filter(e => e.id !== bid)])
            );
            result = Object.fromEntries(Object.entries(result).filter(([, entries]) => entries.length > 0));
            result[pl.date as string] = [...(result[pl.date as string] ?? []), oldEntry];
          }
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (accessToken === "offline-session" || !navigator.onLine) {
      const cached = loadFromCache<DbBooking>(CACHE_KEYS.GENERAL_BOOKINGS);
      if (cached) {
        getAllPending().catch(() => [] as PendingBooking[]).then(pending => {
          // Also add optimistic offline-queued POST bookings to the map
          const map = buildMapFromRaw(cached);
          const postPending = pending.filter(p => !p.method || p.method === "POST");
          for (const p of postPending) {
            const pl = p.payload;
            ...
            const entry: CalBooking = {
              ...
              status:       (pl.status ?? "pending") as CalBooking["status"],
            };
            map[date] = [...(map[date] ?? []), entry];
          }
    ...
    api.get<{ bookings: DbBooking[] }>("/api/vendor/bookings", accessToken)
      .then(res => {
        if (res.success) {
          ...
          setBookingMap(buildMapFromRaw(raw));
          saveToCache(CACHE_KEYS.GENERAL_BOOKINGS, raw);
        }
      })
      .catch(() => {
        const cached = loadFromCache<DbBooking>(CACHE_KEYS.GENERAL_BOOKINGS);
  ...
  const monthEntries     = Object.entries(bookingMap).filter(([k]) => k.startsWith(prefix));
          ...
          <div className="grid grid-cols-7 flex-1" style={{ gridTemplateRows: `repeat(${weeks}, 1fr)` }}>
            {loading ? (
              Array.from({ length: 35 }).map((_, i) => {
                const hasBar = [2, 5, 9, 14, 18, 22, 27, 30].includes(i);
                return (
                  <div key={i} className="p-1.5 lg:p-2" style={{ border: "1px solid #F0F0F0", minHeight: 60 }}>
                    ...
                    {hasBar && <div className="hidden lg:block h-3 rounded-md animate-pulse mb-1" style={{ background: "#E5E7EB", width: "80%" }} />}
                    {hasBar && <div className="hidden lg:block h-2.5 rounded-md animate-pulse" style={{ background: "#F3F4F6", width: "60%" }} />}
                    {hasBar && <div className="flex justify-center gap-1 mt-1 lg:hidden"><div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: "#E5E7EB" }} /></div>}
                  </div>
                );
              })
            ) : cells.map((cell, i) => {
              const bookings    = bookingMap[cell.key] ?? [];
              ...
              let cellBg = "transparent", cellBorder = "transparent";
              if (isPast) {
              ...
              const dayColor = !cell.currentMonth ? "#D1D5DB"
                : isPast    ? "#C4C4C4"
                : onWhite   ? "#ffffff"
                : hasConfirmed ? "var(--primary)"
                : hasPending   ? "#D97706"
                : hasBlocked   ? "#ffffff"
                : "#16A34A";
