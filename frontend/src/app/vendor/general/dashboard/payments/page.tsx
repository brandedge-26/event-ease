        ...
        <div>
          ...
          <div className="flex flex-col gap-0">
            {[
              ...
              <div key={row.label} className={`flex items-center justify-between py-2.5 ${i !== 1 ? "border-b" : ""}`} style={{ borderColor: "#F4F4F5" }}>
  ...
  const loadPayments = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        saveToCache(CACHE_KEYS.GENERAL_BOOKINGS, bookings);
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              ...
              <div className="px-4 py-3">
                <div className="relative">
                  ...
                  <input value={search} onChange={e => changeSearch(e.target.value)} placeholder="Search customer or event…"
                ...
                paginated.map(e => {
                  ...
                  return (
                    <div key={e.bookingId} className="bg-white rounded-2xl shadow-sm p-4" style={isPending ? { border: "2px solid #D97706" } : undefined}>
                      ...
                      {/* Event badge only — no hall badge */}
                      <div className="flex items-center gap-2 mb-3">
                        <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{ background: "var(--primary-light)", color: "var(--primary)" }}>{e.event}</span>
                      </div>
