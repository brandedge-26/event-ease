...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    fetchReports();
  }, [accessToken, range, customMonth]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="mb-5" style={{ maxWidth: "100%" }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="relative shrink-0 inline-flex items-center gap-1">
            ...
            <input ref={monthInputRef} type="month" value={customMonth}
              onChange={e => setCustomMonth(e.target.value)}
              style={{ position: "absolute", top: 0, left: 0, width: "1px", height: "1px", opacity: 0, border: "none", padding: 0, margin: 0 }} />
            {customMonth && (
              <button onClick={() => setCustomMonth("")}
                className="flex items-center justify-center w-6 h-6 rounded-full text-sm font-bold cursor-pointer"
                style={{ color: "#FF3B6B", background: "#FFF0F4", border: "1px solid #FF3B6B" }}>×</button>
            )}
          </div>
        </div>
        ...
        <div className="sm:hidden relative">
          <select value={range} onChange={e => handleRangeChange(e.target.value as Range)}
            className="appearance-none w-full cursor-pointer text-sm font-semibold pl-4 pr-9 py-2.5 rounded-xl border outline-none"
            style={{ background: "#fff", color: "#111827", borderColor: "#E5E7EB", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 mb-3 sm:mb-4">
            ...
            {/* Upcoming Events — no hall, no guests for general vendors */}
                ...
                : (
                  <div className="flex flex-col gap-2">
                    {data.upcoming.map(b => {
                      ...
                      const timeStr = [fmtTime(b.timeFrom), b.timeTo ? fmtTime(b.timeTo) : ""].filter(Boolean).join(" – ");
                      return (
                        <div key={b.id} className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-2xl border" style={{ borderColor: "#F3F4F6" }}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs truncate" style={{ color: "#6B7280" }}>{b.event}</p>
                            {timeStr && <p className="text-[11px]" style={{ color: "#9CA3AF" }}>{timeStr}</p>}
                          </div>
                          <div className="text-right shrink-0">
