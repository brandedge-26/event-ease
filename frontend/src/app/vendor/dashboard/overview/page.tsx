      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Confirmed",  value: s!.confirmed,      note: undefined,                                               dimColor: false },
              { label: "Pending",    value: s!.pending,        note: undefined,                                               dimColor: true  },
              { label: "Cancelled",  value: s!.cancelled,      note: undefined,                                               dimColor: false, red: true },
              { label: "Inquiries",  value: s!.totalInquiries, note: s!.newInquiries > 0 ? `${s!.newInquiries} new` : undefined, dimColor: false },
            ].map(item => (
              <Card key={item.label} className="p-4">
                <p className="text-2xl font-bold" style={{ color: item.red ? "#DC2626" : item.dimColor ? "#6B7280" : "#111827" }}>
                  {item.value}
                </p>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <AreaChart data={data.monthlyChart} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="paid" name="Collected" stroke="#FF3B6B" strokeWidth={2}
                      fill="url(#ovAreaGrad)" dot={false} activeDot={{ r: 4, fill: "#FF3B6B" }} />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </Card>
