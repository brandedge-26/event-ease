...
// ─── PDF (HTML print window — same approach as quotation page) ────────────────
function generatePackagePDF(pkg: Package, vendorName: string) {
...
<title>${pkg.name} – ${vendorName}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .header{display:flex;align-items:flex-start;justify-content:space-between;padding-bottom:20px;border-bottom:2px solid ${pink};margin-bottom:28px}
  ...
  .cat-badge{display:inline-block;margin-bottom:16px;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:600;background:${pinkBg};color:${pink}}
  ...
  .inc-list li::before{content:"✓";color:${pink};font-weight:700}
  ...
  .print-btn{display:block;margin:0 auto 32px;padding:10px 28px;background:${pink};color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer}
</style></head><body>
<button class="print-btn no-print" onclick="window.print()">Download / Print PDF</button>
<div class="header">
  <div><div class="brand">${vendorName}</div><div class="brand-sub">Package Details</div></div>
...
<div class="info-grid">
  <div class="info-card"><div class="lbl">Package Price</div><div class="val" style="color:${pink}">${fmtRs(pkg.price)}</div></div>
  ${pkg.maxGuests ? `<div class="info-card"><div class="lbl">Max Guests</div><div class="val">${pkg.maxGuests}</div></div>` : ""}
  ${pkg.duration  ? `<div class="info-card"><div class="lbl">Duration</div><div class="val" style="font-size:15px">${pkg.duration}</div></div>` : ""}
</div>
${includesHTML}
<div class="footer"><span>Generated by ${vendorName}</span><span>${date}</span></div>
...
function generateAllPackagesPDF(pkgs: Package[], vendorName: string) {
  ...
  const rowsHTML = pkgs.map((p, i) => {
    ...
    return `<tr>
      ...
      <td style="text-align:right">${p.maxGuests ?? "—"}</td>
    </tr>`;
  }).join("");

  const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
<title>Packages Catalog – ${vendorName}</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  body{font-family:'Segoe UI',Arial,sans-serif;color:#111;background:#fff;padding:40px;font-size:13px}
  @media print{body{padding:24px}.no-print{display:none!important}@page{margin:16mm;size:A4}}
...
<button class="print-btn no-print" onclick="window.print()">Download / Print PDF</button>
<div class="header">
  <div><div class="brand">${vendorName}</div><div class="brand-sub">Packages Catalog</div></div>
...
<table>
  <thead><tr>
    ...
    <th>Status</th><th class="r">Price</th><th class="r">Guests</th>
  </tr></thead>
...
export default function PackagesPage() {
  ...
  const vendorName = vendor?.name ?? "EventEase";
    ...
    <>
      ...
      {detailTarget && (
        <DetailDrawer
          ...
          onDownload={() => generatePackagePDF(detailTarget, vendorName)}
        />
      )}
      {deleteTarget && (
        <>
      ...
      <div className="p-4 lg:p-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {packages.length > 0 && (
              <button
                onClick={() => generateAllPackagesPDF(packages, vendorName)}
        ...
        <div className="flex flex-col gap-2">
          {paginated.length === 0 ? (
            ...
            <PackageCard
              ...
              onDownload={() => generatePackagePDF(pkg, vendorName)}
            />
          ))}
        </div>
      ...
      <div className="flex items-center flex-wrap gap-x-4 gap-y-1 mb-3">
        {pkg.maxGuests && (
          <div className="flex items-center gap-1.5">
            <GuestsIcon />
            <span className="text-xs" style={{ color: "var(--fg-muted)" }}>{pkg.maxGuests} guests</span>
          </div>
        )}
        {pkg.duration && (
          <div className="flex items-center gap-1.5">
        ...
        <div>
          ...
          <div className="flex flex-col">
            {[
              ...
              { label: "Max Guests", value: pkg.maxGuests ? `${pkg.maxGuests} guests` : "—" },
    ...
    <>
      ...
      <div
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-5">
          ...
          {/* Pricing */}
          <div>
            <p className="text-[11px] font-bold uppercase tracking-widest mb-3" style={{ color: "var(--fg-subtle)" }}>Pricing & Capacity</p>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="relative">
              ...
              <div>
                <input type="number" placeholder="250" value={f.maxGuests ?? ""}
                  onChange={e => setField("maxGuests", e.target.value ? Number(e.target.value) : null)} min={0}
                  className={inp} style={inpStyle} />
                <p className="text-xs mt-1" style={{ color: "var(--fg-muted)" }}>Max Guests</p>
              </div>
            </div>
            <div className="mt-3">
