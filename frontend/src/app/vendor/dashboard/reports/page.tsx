...
function BarChart({ data }: { data: { label: string; revenue: number; paid: number }[] }) {
  ...
  return (
    <div className="flex items-end gap-1 sm:gap-2 lg:gap-3 h-36 sm:h-40">
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1 sm:gap-1.5 min-w-0">
          <p className="text-[8px] sm:text-[10px] font-bold leading-tight text-center" style={{ color: "#111827" }}>
            {d.revenue > 0 ? fmtRs(d.revenue) : ""}
          </p>
          <div className="w-full relative rounded-t-lg overflow-hidden" style={{ height: 90, background: "#F3F4F6" }}>
            <div className="w-full rounded-t-lg absolute bottom-0 transition-all duration-500"
              style={{ height: `${(d.revenue / max) * 100}%`, background: "#FF3B6B", opacity: 0.18 }} />
            <div className="w-full rounded-t-lg absolute bottom-0 transition-all duration-500"
              style={{ height: `${(d.paid / max) * 100}%`, background: "#FF3B6B" }} />
          </div>
          <p className="text-[8px] sm:text-[10px] truncate w-full text-center" style={{ color: "#9CA3AF" }}>{d.label}</p>
        </div>
      ))}
    </div>
  );
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mb-3 sm:mb-4">
            <Section title="Monthly Revenue" subtitle="Bar = total billed, fill = collected">
              ...
              <div className="flex items-center gap-4 mt-3">
                ...
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-sm" style={{ background: "#FF3B6B", opacity: 0.18 }} />
